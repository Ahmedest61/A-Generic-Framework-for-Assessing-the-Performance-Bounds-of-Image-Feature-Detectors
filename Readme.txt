...
5)	Read Result files
	This function requires a user to input the detector's name (Ibr,Mser etc) and the type of Images/Database (Blur,Light), it then reads the result file of the user's entered detector and database i.e. Ibr_Blur_results. Using the file, it calculates the maximuum, mean and minimum values of the images with same blur/light tranformation and plot it in a form of graph which contains a minimum, mean and a maximum curve of specific database of the detector.

6)   McNemar's Test files
